...
{
   const FontInformation *font = (offscreenBuffer->TextureWidth > offscreenBuffer->WindowWidth) ? (&Proggy32ptFont) : (&Proggy16ptFont);
   DrawNewText(offscreenBuffer, p, font, text, r, g, b, a);
}
...
void DrawNewText(GameOffscreenBuffer *osb, vec2i p, const FontInformation *font, const char *text, float r, float g, float b, float a)
{
   ...
   for (const char *c = text; *c != '\0'; ++c)
   {
      ...
      else if (*c >= font->FirstGlyph && *c <= font->LastGlyph)
      {
